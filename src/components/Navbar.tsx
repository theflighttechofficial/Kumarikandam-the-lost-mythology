...
export function Navbar({ onOpenRealModal, onOpenVsCodeModal, progressPercentage }: NavbarProps) {
  ...
  const navLinks = [
    { name: 'Origins · 1864', href: '#what-is-lemuria' },
    ...
    <header className="sticky top-0 z-40 bg-[#1E1914]/95 backdrop-blur-md border-b border-[#463429] shadow-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <a
            href="#"
            className="flex items-center gap-2.5 text-[#E6D7B9] hover:text-[#FAF6EE] transition-colors group"
          >
            ...
            <div>
              ...
              <span className="hidden sm:inline-block text-[9px] font-carto text-[#9A7B45] ml-2 px-1.5 py-0.5 rounded bg-[#2B211A] border border-[#463429] uppercase tracking-widest">
                Cartographic Survey
              </span>
            </div>
          </a>
          ...
          {/* Desktop Nav Links */}
          <nav className="hidden lg:flex items-center gap-5 text-xs font-carto uppercase tracking-wider text-[#CDBB96]/80">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="hover:text-[#FAF6EE] transition-colors py-1 hover:border-b border-[#9A7B45]/70"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <a
              ...
              className="flex items-center gap-2 px-2.5 sm:px-3 py-1.5 text-xs font-semibold rounded-full bg-[#2B211A] text-[#E6D7B9] border border-[#463429] hover:border-[#9A7B45] transition-colors shadow-inner"
              title="View Field Journal & Expedition Tasks"
            >
              <CheckSquare className="w-3.5 h-3.5 text-[#9A7B45]" />
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-semibold rounded bg-[#9A7B45] hover:bg-[#8B5E4A] text-[#FAF6EE] shadow transition-all active:scale-95 font-heading tracking-wide border border-[#CDBB96]/40"
            >
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded bg-[#2B211A] hover:bg-[#463429] text-[#CDBB96] border border-[#463429] hover:border-[#756451] transition-colors"
